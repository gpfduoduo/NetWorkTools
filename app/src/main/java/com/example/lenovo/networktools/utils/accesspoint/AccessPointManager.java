...
public class AccessPointManager extends WifiManagerWrap {
	...
	private BroadcastReceiver mWifiApStateReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (WIFI_AP_STATE_CHANGED_ACTION.equals(action) 
				...
				mOnApStateChangeListener.onWifiStateChanged(intent.getIntExtra(EXTRA_WIFI_AP_STATE, WIFI_AP_STATE_FAILED));
			}
		}
	};
	...
	public boolean startWifiAp() {
		...
		if (Constant.isHTC()) { // 2013-12-31 add special for htc
			enableHtcHotspot(getWifiApConfiguration(), false);
			return setHtcHotspot();
		}

		if (isWifiApEnabled()) setWifiApEnabled(getWifiApConfiguration(), false);
	...
	private boolean setHtcHotspot() {
		...
		if (!setHTCSSID(apConfig)) return false;
	...
	public boolean setHTCSSID(WifiConfiguration config) {
		...
		{            
			...
			if (localoObject1 != null)
			{
				...
				Field localField3 = localoObject1.getClass().getDeclaredField("dhcpEnable");
				...
				Field locaField4 = localoObject1.getClass().getDeclaredField("secureType");
	...
	public void stopWifiAp() {
		...
		//close WiFi hot spot
		if (isWifiApEnabled()) setWifiApEnabled(getWifiApConfiguration(), false);
		else enableHtcHotspot(getWifiApConfiguration(), false);
	}
